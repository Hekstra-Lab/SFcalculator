...
def r_factor(Fo, Fmodel, rwork_id, rfree_id):
    ...
    Parameters
    ----------
    Fo: tf.tensor, [N_hkl,], complex
        1D tensor containing Fo corresponding to HKL list
    Fmodel: tf.tensor, [N_hkl,], complex
    ...
    '''
    R_work = tf.reduce_sum(tf.abs(tf.gather(
        Fo, rwork_id) - tf.gather(Fmodel, rwork_id))) / tf.reduce_sum(tf.gather(Fo, rwork_id))
    R_free = tf.reduce_sum(tf.abs(tf.gather(
        Fo, rfree_id) - tf.gather(Fmodel, rfree_id))) / tf.reduce_sum(tf.gather(Fo, rfree_id))
...
def DWF_iso(b_iso, dr2_array):
    ...
    '''
    return tf.cast(tf.exp(-tf.reshape(b_iso, [-1, 1])*dr2_array/4.), dtype=tf.float32)
...
def DWF_aniso(b_aniso, reciprocal_cell_paras, HKL_array):
    ...
    DWF_aniso_vec = tf.exp(log_value)
    return tf.cast(DWF_aniso_vec, dtype=tf.float32)
...
def vdw_rad_tensor(atom_name_list):
    ...
    for atom_type in unique_atom:
        ...
        vdw_rad_dict[atom_type] = tf.constant(element.vdw_r, dtype=tf.float32)
    vdw_rad_tensor = tf.convert_to_tensor(
        [vdw_rad_dict[atom] for atom in atom_name_list])
    return vdw_rad_tensor


def vdw_distance_matrix_tf(atom_name_list):
    '''
