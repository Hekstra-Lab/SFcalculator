...
def packingscore_voxelgrid_torch(atom_pos_orth, unit_cell, space_group, vdw_rad, unitcell_grid_center_orth_tensor, CUTOFF=0.0001):
    '''
    ...
    spacing = torch.max(unitcell_grid_center_orth_tensor[1] - unitcell_grid_center_orth_tensor[0])
    # s ~ log(1/c -1) / (d/2 - r)
    spacing = torch.maximum(spacing, torch.tensor(3.5))
    steepness = np.log(1.0/CUTOFF - 1.0)/(spacing/2.0 - 1.5)

    voxel_map_p1 = voxelvalue_torch_p1(unitcell_grid_center_orth_tensor, atom_pos_orth,
